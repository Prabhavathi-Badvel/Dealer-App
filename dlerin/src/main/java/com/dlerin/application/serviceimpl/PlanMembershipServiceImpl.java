...
public class PlanMembershipServiceImpl implements PlanMembershipService {
	...
	public PlanMembership addPlanMembership(PlanMembership planMembership) {
		Optional<AdminLogin> login = Optional.ofNullable(
				adminLoginRepo.findByEmpId(planMembership.getUpdatedBy()));
		if (login.isPresent()) {
	...
	public PlanUpdateResponse updatePlanAndStoreMembership(PlanMembership planMembership) {
	    // Step 1: Retrieve the existing PlanMembership using the planId from the input object
	    ...
	    if (planOpt.isPresent()) {
	        ...
	        List<StoreMembership> storeMemberships = storeMembershipRepo.findByStoreCurrentPlan(planMembership.getPlanId());

	        // Check if any StoreMembership records were found
	        if (storeMemberships.isEmpty()) {
	            throw new EntityNotFoundException("No StoreMembership records found for plan ID " + planMembership.getPlanId());
	        }
